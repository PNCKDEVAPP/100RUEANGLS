...
let title_set = line.replace(/[^\w\s]/g,'').replace(/staticphotoview/g,'100เรื่อง-น้องสาวเอเชีย_')
...
date: ${yourDate}
image: "photo/${line.split("/")[2]}/${files.randomElement()}"
type: "regular"
draft: false
---

{{< foldergallery src="photo/${line.split("/")[2]}/" alt="${title_set} ${textstats.randomElement()}" >}}
...
fs.writeFile(`content/english/blog/100gallery/${title_set.replace(/_/g,'')}.md`, mdpost, 'utf8', function (err) {
    if (err) {
        console.log("An error occured while writing JSON Object to File.");
        return console.log(err);
    }
